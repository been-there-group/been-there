...
const IndividualPlaces = (props) => {
  ...
  const saveToBucketList = ()=> {
    axios.post(`/api/save/`, {place_id: props.places.place_id, place_name: props.places.name, bucket_list_image: `https://cors.bridged.cc/https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${props.places.photos[0].photo_reference}&key=${key}`})
      ...
      <div className='single-result-bottom'>
        ...
        <div className='single-result-bottom-left'>
          <section className="name-and-rating">
            ...
            <p className="place-type">Type: {props.places.types[0]}</p>
            <p className="place-status">Status: {props.places.business_status === 'OPERATIONAL' ? 'Open' : 'Closed'}</p>
